...
### Repair после remote CI FAILED: timeout в cleanup Stage 9 stress-теста

Remote GitHub Actions на HEAD `07e4f90` (Stage 10 gate) — **FAILED**.
Падение — не в новом Stage 10 коде (read-only `/audit-log` его не трогает),
а в `scripts/check-stage9-e2e.mjs` внутри `pnpm test:docker --browser`:
stress-прогон 11/20 (`workers=2, retries=0`), тест `Stage 9: повторный
импорт того же файла — вероятные дубли по умолчанию пропускаются`, 3/4
теста файла прошли; упавший тест истёк по таймауту (30000 мс) не в
собственных шагах, а в `finally`-cleanup — `cleanupBySearch()` →
`page.request.get('/api/v1/transactions?...&pageSize=50')` не дождался
ответа. До этой remote-попытки 20/20 stress проходили (см. запись Stage 9
выше) — регрессия воспроизводилась только на remote-раннере/под нагрузкой,
не локально.

#### Root cause

Не продуктовый дефект и не порча тестового окружения между прогонами — это
дефект изоляции E2E-гарнитуры: **непреднамеренное разделяемое mutable
state между параллельными Playwright workers**.

Все 4 теста `csv-import.compose.spec.ts` аутентифицируются одной и той же
заранее засеянной демо-учётной записью `personal@finora.example`
(`sessions.personal` из общего `compose-auth` setup, `apps/web/e2e/compose-session.ts:35-49`,
`apps/web/e2e/compose.setup.ts:32-44`) — общий паттерн для всех
`*.compose.spec.ts`, оправданный экономией login rate limit (10/60 с).
Каждая финансовая мутация этого аккаунта — `create`/`update`/`remove`
операции и `POST /imports` commit — обязана взять
[`lockOwner`](../apps/api/src/modules/finance/locking.ts) (`SELECT id FROM
users WHERE id = $1::uuid FOR UPDATE`) ради корректности конкурентных
мутаций одного реального пользователя (уже описано и осознанно принято в
Concurrency review Stage 9 выше — но тогда рассматривались только два
одновременных `POST /imports` от одного пользователя, а не параллельные
Playwright workers, притворяющиеся одним пользователем).

При `--workers=2/4` тесты этого файла реально выполняются одновременно, но
все пишут от имени одной и той же строки `users` — их `lockOwner`-вызовы
сериализуются друг за другом на одной и той же блокировке, и каждый из них,
пока ждёт снятия блокировки другим тестом, продолжает удерживать одно из
ограниченного числа (`pg.Pool` default `max: 10`,
`apps/api/src/prisma/client.ts:5-11`) соединений API-процесса с БД — Prisma
interactive `$transaction` держит закреплённое соединение на всё время
callback, включая ожидание `FOR UPDATE`. Упавший `GET /api/v1/transactions`
сам блокировку не берёт (`transactions.service.ts` `list()` — обычный
`SELECT`, без `lockOwner`), но чтобы выполниться, ему тоже нужно получить
соединение из того же пула — если в моменте несколько write-транзакций
других тестов застряли в очереди на `FOR UPDATE`, каждая удерживая
соединение, эта одна GET-операция может не получить свободное соединение
вовремя.

**Доказательство механизма (не только вывод по анализу кода).** Поднят
одноразовый Compose-стек (`docker compose -p finora-repro`, тот же образ, что
собирает `test-docker.mjs`), контейнеры `api`/`postgres` намеренно
CPU-throttled (`docker update --cpus=0.5`) для эмуляции более медленного/
загруженного remote-раннера. Прямой поллинг `pg_stat_activity` во время
`csv-import.compose.spec.ts --workers=2` зафиксировал (полный лог сохранён
в сессии) момент, когда **два разных backend PID одновременно находятся в
`state=active, wait_event_type=Lock`** на идентичном запросе `SELECT id
FROM users WHERE id = $1::uuid FOR UPDATE` — то есть реальная, не
гипотетическая, сериализация двух тестов на одной блокировке под нагрузкой.
Довести локальный прогон до полных 30 с таймаута не удалось — 25
последовательных и 3×12 параллельных локальных прогонов (0.5 vCPU throttle)
и полный официальный `check-stage9-e2e.mjs` (20/20, включая throttled
прогон) на этой машине прошли за 2–5 с на итерацию: у машины с 10
ядрами и тёплым Docker Desktop даже при cgroup-ограничении контейнеров
остаётся достаточно host-параллелизма, чтобы очередь на блокировку
разрешалась за миллисекунды. Это согласуется с тем, что дефект
воспроизводился только на remote CI/под stress, а не локально — не
опровергает и не заменяет доказанный факт самой сериализации, а объясняет,
почему её продолжительность на конкретной remote-машине способна
дотянуться до 30 000 мс, тогда как здесь — нет.

Проверены и отклонены (без подтверждающих доказательств) альтернативные
гипотезы: rate limiting (guard применяется только к
`POST/PATCH/DELETE` с явным `@AuthRateLimit`, `GET` не участвует —
`apps/api/src/modules/auth/security.guard.ts:28`); накопление данных между
stress-итерациями (`resetApi()` перезапускает только `api`, не `postgres`,
но переисполняемый на каждом старте `dist/prisma/seed.js` идемпотентен и
после первого запуска завершается одним `SELECT` по completion-маркеру —
`apps/api/prisma/seed-database.ts:17-24` — не источник нагрузки); recurring
scheduler (у `personal` `nextOccurrenceDate` всех правил после seed —
следующий месяц после `SEED_ANCHOR_DATE`, то есть в будущем относительно
даты прогона — ни одного due-правила, `runSchedulerTick` кандидатов не
находит).

Классификация: **дефект E2E/cleanup-гарнитуры (test isolation), не
продуктовый дефект.** Серверный `lockOwner` работает корректно и
обязателен для реальной конкурентной записи одного пользователя — сужение
или удаление этой блокировки было бы неверным исправлением. Также не
clean-runner/portability-дефект — воспроизводится на любом раннере при
достаточной нагрузке/медленности, не зависит от ОС/архитектуры.

#### Исправление

`apps/web/e2e/csv-import.compose.spec.ts` — тесты файла больше не
переиспользуют общий `sessions.personal`. Добавлен собственный,
переопределяющий базовый `sessions` fixture, worker-scoped fixture
`csvOwner`: на каждый Playwright worker (не на каждый тест — не расходовать
впустую auth register rate limit 5/час) регистрируется отдельный,
независимый аккаунт (`baseCurrency: 'RUB'`, тот же, что у `personal`, —
сохраняет исходную семантику теста без шага ввода курса валют;
`timeZone: 'Europe/Moscow'`). `finance` auto-fixture (используется тестом
экспорта) зависит от того же `sessions` fixture, поэтому автоматически
использует тот же изолированный аккаунт — идентичность браузерной сессии и
API-записи остаётся согласованной без дополнительных правок.

Это устраняет саму причину, а не симптом: тесты этого файла больше не
делят ни одну строку `users`, поэтому `lockOwner` разных тестов никогда не
конкурирует за одну и ту же блокировку и не удерживает соединения пула друг
у друга — race устранена структурно (lifecycle-изоляция), а не замаскирована
таймаутом/retry. `retries` остаётся `0`; строгость проверки дублей (`Будет
импортировано: 0`, `Вероятные дубли: 1`, explicit opt-in) не изменена;
stress-прогон (20 итераций, workers 1/2/4) не удалён и не сокращён;
timeout самого запроса не увеличивался.

#### Регрессионное доказательство

- Упавший тест изолированно (`-g "повторный импорт"`, `--workers=1`): PASS.
- Полный `csv-import.compose.spec.ts` (`--workers=4`, все 4 теста файла,
  включая тест экспорта, зависящий от `finance`-fixture): PASS, 4/4.
- `node scripts/check-stage9-e2e.mjs` (тот же CPU-throttled Compose-стек,
  что использован для доказательства механизма выше): **PASS, 20/20**,
  `workers=1/2/4` по циклу, `retries=0`, без единого timeout/flaky —
  включая итерацию 11 с `workers=2` (та же комбинация, что упала на remote
  CI).
- `pnpm test:docker --browser` (чистый `git ls-files`-checkout во
  временную директорию — тот же clean-runner/clean-checkout сценарий, что
  собирает remote CI, независимый одноразовый Compose-проект): **PASS**
  целиком, exit code 0 — все стадии (Stage 5/6/Dashboard/8/outage/9/10
  Docker acceptance, Stage 7 dashboard-stress 20/20, встроенный Stage 9
  stress 20/20 включая итерацию 11/20 workers=2) зелёные, dataset SHA-256
  стабилен, ни одного `✘`/`failed`/`Error` в полном логе прогона (кроме
  ожидаемого управляемого сбоя в Stage 8 outage-тесте).
- `lint` / `format:check` / `typecheck` (`pnpm -r run typecheck`): PASS.
- `git status --short` после исправления: изменён только
  `apps/web/e2e/csv-import.compose.spec.ts`; посторонних артефактов нет.

Commit/push не выполнялись — по инструкции этой сессии. Stage 11 не начат.
...
API и (повторно) БД. Дополнительно в этой сессии устранён remote CI
FAILED, вызванный дефектом изоляции E2E-гарнитуры Stage 9
(`csv-import.compose.spec.ts` — общий `personal` аккаунт между параллельными
Playwright workers), см. repair выше; сам Stage 10 scope при этом не
менялся. Stage 11 не начат. Commit и push не выполнялись в этой
...
## 15.09.2026 — Stage 12: Deployment & Demo Preparation

Исходная точка: `git status --short` в начале сессии — чисто; HEAD `f85d6ed`
(Stage 11, см. запись выше). Пользователь подтвердил, что `f85d6ed` — green
baseline (local validation + clean-runner + remote CI); эта сессия
дополнительно перепроверила заявление напрямую через `gh run list`:
run `35013939758` («chore: усилить безопасность и завершить локальную
проверку Stage 11»), `conclusion: success`, `2026-09-15T19:29:32Z` —
подтверждено на GitHub-hosted Linux x86_64. До реализации прочитаны
AI_RULES.md целиком, раздел Stage 12 ROADMAP.md, разделы ARCHITECTURE.md
§22–24 (Seed, Docker и развертывание, CI и история разработки) и текущий
README.md полностью (единственный источник фактических deployment/demo
команд).

### Scope

Stage 12 по ROADMAP — не новая функциональность, а: (1) финальная проверка
чистого клона и demo без ручной настройки, (2) финальный README с реальными
командами/адресами/credentials, (3) сверка ARCHITECTURE/ROADMAP с
реализацией, (4) сценарий демонстрации в README, (5) итоговый git status/
history. Video, облачный hosting, Kubernetes и продуктовые изменения — вне
scope. Реализация ограничена документацией; код не менялся, так как
clean-runner acceptance и полный локальный gate прошли без единого дефекта
(см. ниже) — блокирующих дефектов, требующих исправления перед сдачей, не
обнаружено.

### Найденные и устранённые несоответствия документации

Реального продуктового/deployment дефекта не найдено. Найдены и исправлены
только несоответствия документации фактической реализации (сверка
ARCHITECTURE/ROADMAP, обязательная по Stage 12):

1. **README, вводный абзац и раздел «CI»** — фиксировали состояние на уровне
   Stage 9 («Stage 9 реализован... LOCAL VALIDATION COMPLETE») несмотря на
   то, что Stage 10 и Stage 11 уже реализованы, закоммичены и подтверждены
   зелёным remote CI. Исправлено: вводный абзац упоминает Stages 1–11 и
   Stage 12; раздел «CI» ссылается на актуальный диапазон коммитов
   (`bb3ff1a` … `f85d6ed`) и текущий подтверждённый зелёный HEAD, детали —
   в REPORT.md.
2. **ARCHITECTURE.md, строка статуса (раздел 1)** — перечисляла Stage 1–10
   как реализованные, не упоминая Stage 11. Исправлено: добавлено Stage 11
   (Hardening & Testing — targeted review, без изменения контрактов).
3. **ROADMAP.md, строка статуса** — застряла на состоянии «Stage 10 не
   закоммичен... Stage 11–12 не начаты», хотя Stage 10 и Stage 11 уже
   закоммичены и подтверждены зелёным remote CI. Исправлено: строка статуса
   обновлена до диапазона `848c384` … `f85d6ed` с отсылкой к REPORT.md за
   детальной историей remote CI regressions/repair по каждому Stage.
4. **README — отсутствовал явный сценарий демонстрации**, требуемый Stage 12
   ROADMAP («сценарий демонстрации в README»). Добавлен раздел
   «Демонстрация (5 минут)» с шагами, которые физически пройдены в этой
   сессии (см. ниже), а не только описаны.

Ни один из этих пунктов не менял факты, историю или структуру Stage-разделов
README/ARCHITECTURE/ROADMAP — только актуализировал статусные формулировки
до состояния, уже зафиксированного в REPORT.md по факту завершения Stage
10/11.

### Демо-репетиция (физически пройдена в этой сессии)

На собранных production-образах (`docker compose up -d --build`, без
`--build` кеша от предыдущих Stage-сессий не переиспользовалось — images
пересобраны заново) поверх уже существующего dev-volume PostgreSQL:

- `/health/live`, `/health/ready` — `{"status":"ok"}`; `/` — HTTP 200;
  `/docs` (Swagger) — HTTP 200 и открыт визуально, реальный OAS 3.0 контракт.
- Вход demo-кнопкой «Личный профиль» (`personal@finora.example` /
  `Finora-Personal-2026!`) — Обзор с реальными KPI (доходы/расходы/разница/
  доля сбережений).
- **Транзакции** — список, фильтры, кнопки «Импорт CSV»/«Экспорт CSV»
  отрисованы и доступны.
- **Бюджеты** — сентябрь 2026: категория с превышением лимита («Кафе и
  рестораны», 125% использовано) и категория около лимита («Продукты», 95%)
  видны на первом экране.
- **Регулярные операции** — активные MONTHLY-правила (зарплата, аренда,
  подписки) с датой следующей операции.
- **Журнал изменений** (`/audit-log`) — список записей с фильтрами и кнопкой
  «Подробнее».
- **Создание операции с проверкой отклика**: на Обзоре Марии (family)
  добавлена тестовая операция «Демо-проверка Stage 12» (1000 RUB, расход,
  категория «Продукты») — «Расходы» немедленно обновились с 220 245,94 до
  221 245,94 RUB, «Разница» — с −1 745,94 до −2 745,94 RUB без перезагрузки
  страницы. Операция затем найдена на `/transactions`, удалена через
  подтверждение удаления; Обзор вернулся к исходным 220 245,94 RUB —
  demo-датасет не искажён.
- **Logout → вход вторым demo-профилем**: кнопка «Выйти» вернула на `/login`
  с обеими demo-кнопками и видимыми публичными credentials; вход «Семейный
  профиль» (`family@finora.example` / `Finora-Family-2026!`) показал
  независимые данные второго владельца (другие суммы KPI) — независимость
  двух demo-аккаунтов подтверждена вручную, не только тестами.
- Контейнеры `api`/`web` остановлены (`docker compose stop api web`) после
  проверки; ранее существовавший `postgres` (dev-окружение сессии) не
  затронут.

### Локальные проверки (после всех правок этой сессии)
...
| `pnpm typecheck`                                  | PASS                                |
| Backend tests (`pnpm --filter @finora/api test`)  | PASS 137/137                        |
| Frontend tests (`pnpm --filter @finora/web test`) | PASS 97/97 (9 файлов)               |
| `pnpm build`                                      | PASS (api + web)                    |
| `pnpm db:validate`                                | PASS                                |
| `pnpm api:check`                                  | PASS (OpenAPI/Orval воспроизводимы) |
| `pnpm test:e2e:auth` (clean-runner + browser E2E) | PASS, exit code 0                   |

Числа тестов идентичны Stage 11 baseline (137/137 backend, 97/97 frontend) —
ожидаемо, поскольку Stage 12 не менял application code.

### Clean-runner acceptance — детали

`pnpm test:e2e:auth` (= `test:docker --browser`): чистая копия tracked-files
во временную директорию, отдельный одноразовый Compose-проект на случайных
портах, без `.env`/host `node_modules`/локальных build artifacts. Результат —
**PASS, exit code 0**, 0 упоминаний `FAIL` в полном логе (601 строк, 254
проверенных `✓`-утверждений Playwright):

- Stage 5/6/Dashboard/8 (включая outage-тест недоступности PostgreSQL во
  время recurring tick)/9/10 Compose acceptance — PASS;
- полный Playwright-browser набор (auth, finance, budgets, dashboard, CSV
  wizard/export/mobile, responsive/axe) — PASS;
- **Stage 9 stress 20/20** (workers 1/2/4, retries=0, 80 test cases) — без
  единого flake;
- **Stage 7 dashboard-stress 20/20** (workers 1/2/4, repeat-each=2,
  retries=0, 120 critical cases) — без единого flake;
- итоговый dataset SHA-256 после повторного restart/seed —
  `55c7d9a51be58a2b6d685feb3d3057333c2dfd7fe6be729cbce3bf436a4c89b0` —
  **идентичен** значению, зафиксированному в Stage 11 (детерминированность
  seed подтверждена между сессиями, не только между restart одной сессии);
- acceptance-окружение и его volume удалены runner'ом по завершении.

Новый regression-тест не добавлялся: реального продуктового дефекта не
найдено, только устаревшие формулировки в документации.

### Git-история и remote CI

`git log --oneline` — 21 коммит, `bb3ff1a` (архитектурная документация)
предшествует `b76f546` (первый application code, foundation), что
соответствует ARCHITECTURE.md §24. История последовательна по Stage,
включая коммиты-исправления после remote CI failures (`93566e0` после
`848c384`, `7f4cdd8` после `07e4f90`) — это не скрывается, а прослеживается
через `gh run list` и REPORT.md записи Stage 8/10/11. Remote `origin` —
`github.com/pw5rhn4tnn-dotcom/finora`, публичный доступ подтверждён `gh
auth status`. HEAD `f85d6ed` подтверждён зелёным (см. выше).

### Self-review изменённого Stage 12 diff

`git diff --stat` — только `README.md`/`ARCHITECTURE.md`/`ROADMAP.md`
(39 insertions, 14 deletions), без изменений кода/схемы/конфигурации.
Проверено: portable deployment не затронут (docker-compose.yml не менялся);
все документированные в новом разделе «Демонстрация» шаги физически
пройдены в этой сессии, а не только описаны; секретов не добавлено (все
credentials — уже публичные demo-значения из существующего `.env.example`/
seed); детерминированность инициализации подтверждена совпадением dataset
SHA-256 с Stage 11; скрытой зависимости от локальных артефактов не внесено
(правки — только Markdown-текст).
...
#### Remote CI recheck: подтверждён зелёным, затем новый регресс на CSS-only коммите (следующая сессия)

Пользователь закоммитил и запушил timeout-policy fix самостоятельно —
`d029733` («fix: устранить гонку таймаутов PostgreSQL в Stage 12»). Run
`35084398614` — **FAILED за 3m59s** (слишком быстро для полного E2E-гейта;
упал на раннем шаге, не на browser E2E). Следующий коммит `7d48d25`
(«fix(stage-12): исправить форматирование после проверки CI») —
**SUCCESS, 18m34s**, полный гейт, включая browser E2E и встроенный Stage 9
stress — то есть сам timeout-policy fix (`lock_timeout`/`statement_timeout`,
без клиентского `query_timeout`) подтверждён зелёным remote CI целиком,
«REMOTE CI RECHECK PENDING» закрыт как **PASSED**.

Затем пользователь закоммитил и запушил `94e9f28` («fix: выровнять размеры
выпадающих списков и пагинации») — **только 2 CSS-файла**
(`git show --stat 94e9f28`: `apps/web/src/**/*.css`, без единого
`.ts`/`.tsx`/backend-файла). Run `35093096605` — **FAILED, 10m34s**, тот же
самый тест, что и во всех предыдущих repair-записях этого Stage:
`csv-import.compose.spec.ts:164` («Stage 9: повторный импорт того же
файла — вероятные дубли по умолчанию пропускаются»), тот же симптом
(`apiRequestContext.get: Test timeout of 30000ms exceeded` в
`cleanupBySearch()`, `csv-import.compose.spec.ts:78`, вызов из `finally` на
строке 216). Пользователь прямо потребовал: не считать «просто перезапустить
CI» ответом, не объявлять CSS причиной без доказательства, найти
воспроизводимое root cause и устранить недетерминированность, если она
подтверждается — со списком из 10 обязательных пунктов проверки.

**1. CSS исключён как причина — доказательством, не предположением.**
`git show --stat 94e9f28` и `git diff e229dff..94e9f28 --
'apps/web/e2e/**' 'apps/api/**'` — пусто: единственные тронутые файлы —
`apps/web/src/components/**/*.css` (выравнивание ширины select/пагинации),
ноль изменений в `apps/web/e2e/csv-import.compose.spec.ts`, ноль изменений в
`apps/api/**`, ноль изменений в `apps/api/src/prisma/client.ts`
(timeout-policy fix из `d029733`/`7d48d25` — без изменений с прошлой
сессии). Приложенный и упавший тест физически не рендерит и не читает
CSS-классы, которые правит этот diff (импорт-визард, не список/пагинация).
CSS — не причина; совпадение по времени коммитов, не по коду.

**2. Точная реконструкция таймлайна по логам job'а (не предположение —
timestamp-корреляция).** GitHub Actions буферизует `docker compose logs`:
все строки `api-1` в скачанном логе идут с одним и тем же GHA-timestamp
(момент flush при остановке контейнера), но КАЖДАЯ строка несёт собственный
`pid`-timestamp (`Date.now()` внутри самого Node-процесса) — это и есть
настоящее время события. Извлечены и отсортированы по этому полю все
HTTP-access-log записи API (`context: "HTTP"`) за окно вокруг падения
(`csv-import.compose.spec.ts:164` упал через 30.2с, отчёт в 12:07:47.577Z ⇒
старт ≈12:07:17.4Z). Результат:

- Последний настоящий (не `/health/ready`) HTTP-запрос, завершившийся
  успехом, зафиксирован в 12:07:20.095Z (`DELETE ... 204`).
- С 12:07:20.095Z по 12:07:46.163Z (конец окна выгрузки) — **ни одной**
  записи `context: "HTTP"` ни для одного маршрута, КРОМЕ `/health/ready`,
  которые продолжают приходить каждые ~1.3–1.7с СО СТАБИЛЬНОЙ латентностью
  1–13мс на всём интервале.
- `HealthController.ready()` (`apps/api/src/health/health.controller.ts:58`)
  идёт через `PrismaService.isReady()` → `this.client.$queryRaw\`SELECT 1\``— **тот же самый** расшаренный`PrismaClient`/pg-pool, что и
`TransactionsService.list()`(обработчик`GET /transactions`, вызванный из
`cleanupBySearch()`), не отдельное соединение/клиент.

Из этого следует однозначно (не гипотеза, а исключение по прямой улике):
API-процесс не завис (event loop свободен — health check каждый раз получает
управление и отвечает за единицы миллисекунд), пул соединений НЕ пуст/не
полностью занят (health check каждый раз получает соединение мгновенно), и
Postgres отвечает на `SELECT 1` без задержек весь интервал. Зависший `GET
/transactions` не оставил вообще никакого следа в access-логе за ~26 секунд
— ни успеха, ни ошибки.

**3. Все ограниченные по времени серверные механизмы исключены по величине
несоответствия.** Каждый из них по конструкции обязан либо вернуть ответ
(успех/ошибка), либо истечь заметно раньше 26 секунд:

- `connectionTimeoutMillis: 3000` (`apps/api/src/prisma/client.ts:12`) —
  ограничивает ожидание СВОБОДНОГО соединения в очереди `pg-pool`
  (`node_modules/.pnpm/pg-pool@3.14.0.../lib/index.js:190-231`, подтверждено
  чтением исходника в этой сессии: `connect()` ставит `setTimeout` именно на
  элемент `_pendingQueue`, не только на установление НОВОГО TCP-соединения).
  Если бы пул был полон, вызывающий получил бы ошибку `timeout exceeded
when trying to connect` через 3с — с логируемым ответом. Не совпадает с
  26с тишины.
- `lock_timeout: 5000` — ограничивает ожидание Postgres-блокировки; `GET
/transactions` — обычный `SELECT` без `FOR UPDATE`
  (`TransactionsService.list()`, `transactions.service.ts:101-121`), не
  вызывает `lockOwner()`, и даже если бы упёрся в блокировку — 5с, не 26с, и
  тоже с логируемой ошибкой `55P03`.
- `statement_timeout: 35000` — теоретически мог бы объяснить сам факт
  ожидания дольше 30с (Playwright сдался раньше него), но не объясняет
  ПОЧЕМУ запрос к таблице `transactions`, отфильтрованный по индексированному
  `userId` worker-изолированного `csvOwner`-аккаунта (у которого от силы
  1–2 своих строки — `@@index([userId, transactionDate, id])`,
  `schema.prisma:101`), в принципе выполнялся бы секундами на своём backend:
  объём данных на строку не соответствует такой длительности.

**4. Обе уже известные причины «зомби-соединения» — перепроверены на
установленной версии `@prisma/client@7.10.0` и подтверждены закрытыми (не
предположение — чтение скомпилированного `runtime/client.js` в этой сессии).**
Кроме уже задокументированного механизма `query_timeout` (убран в
`d029733`), в этой сессии отдельно проверен смежный кандидат: гонка между
Prisma `maxWait` (дефолт 2000мс для интерактивных транзакций без явного
`timeout`/`maxWait` — используют `TransactionsService.list()` и
`ImportsService.validate()`) и `connectionTimeoutMillis` (3000мс) — если
получение соединения из пула занимает 2000–3000мс, Prisma по `maxWait`
могла бы «сдаться» раньше, чем `pg-pool` реально вернёт соединение, оставив
его в неопределённом состоянии. Чтение `TransactionManager.#i()` (метод
`start_transaction`) в `runtime/client.js` показало: при срабатывании
`maxWait` (`n.abort()`) Prisma НЕ бросает реальную попытку подключения — она
запускает фоновый `#o(a)`, который дожидается настоящего результата
`driverAdapter.startTransaction()` и, если он всё же прибывает, сама
выполняет `rollback()`/`release()` для него. Утечки нет и на этом пути —
кандидат закрыт по коду, не по вере в документацию.

**5. Вывод: зависание — не на стороне сервера.** Все server-side
объяснения, ограниченные по времени, либо не совпадают по величине (3с/5с
против 26с), либо не могли бы пройти незамеченными в access-логе (ошибка
логируется так же, как успех). `/health/ready` на том же самом пуле/клиенте
доказывает: процесс, пул и Postgres были живы и отзывчивы весь интервал.
Единственное место, где HTTP-запрос может «потеряться» на 26 секунд без
единого серверного следа — до того, как он реально ушёл на сервер: на
самом раннере, где `--workers=2` держит ДВА параллельных процесса Chromium
одновременно с Node API и Postgres на стандартных 2 shared vCPU
`ubuntu-latest`; если ОС на конкретный момент не выделяет CPU-время именно
тому процессу Chromium, который должен отправить `page.request.get(...)`
для упавшего воркера, диспетчеризация запроса откладывается без всякого
server-side лога — ровно до того момента, когда планировщик наконец отдаст
этому процессу время (что здесь произошло позже 30-секундного бюджета
Playwright). Дисбаланс между воркерами такому объяснению не противоречит:
«быстрый» воркер (tests 100/220/266) успел получить процессорное время
раньше и полностью завершился к 12:07:20.095Z; «медленный» воркер (test 164)
— нет.

**Честно о границах доказательства.** Это — наиболее обоснованное
объяснение по методу исключения (каждая альтернатива опровергнута прямой
уликой из логов/исходников), а не воспроизведённый на месте факт: как и в
двух предыдущих сессиях этого Stage (искусственный CPU-throttle 0.5 vCPU,
Stage 10 repair), в этой сессии тоже не удалось детерминированно
воспроизвести именно 26-секундное зависание ни локально, ни под нагрузкой
(см. п. 7 ниже) — шум планировщика shared-раннера GitHub Actions
принципиально не воспроизводим по требованию в локальной песочнице. Это
согласуется с классификацией «дефект нагрузки remote-раннера», а не
опровергает её.

**6. Почему предыдущий CI (`7d48d25`) был зелёным, а этот (`94e9f28`) —
красным, при неизменном коде теста/приложения.** Предпосылка зависания —
конкретная по времени комбинация: OS-планировщик shared 2-vCPU раннера не
выделяет CPU-время нужному Chromium-процессу в тот момент, когда пул из двух
Playwright worker'ов конкурирует с Postgres и Node API. Это — свойство
конкретного запуска конкретной VM в конкретный момент (совместно
используемая инфраструктура GitHub Actions, «шумные соседи»), не свойство
кода. Одинаковый код (тест и приложение между `7d48d25` и `94e9f28` не
менялись — единственный diff, п. 1) на разных запусках попадает на разные
хосты/нагрузку — «зелёный → красный → (ожидаемо) снова зелёный» без единого
изменения тестового/серверного кода — это и есть ожидаемая подпись именно
такого класса дефекта, а не свидетельство скрытой регрессии.

**7. Почему CSS-коммит «проявил» проблему, хотя CSS её не создавал.** Не
проявил — совпадение по времени коммитов и совпадение по нагрузке раннера в
конкретный момент, не причинно-следственная связь. `git diff` (п. 1) и
структура диагноза (п. 2–5) не оставляют места для влияния CSS на этот
тест: класс дефекта (OS-планировщик shared раннера) не зависит и не может
зависеть от содержимого `.css`-файлов, не участвующих в рендере
CSV-импорт-визарда и не влияющих на API/БД.

**8. Структурное исправление (пункт 8 задания пользователя): отделить
детерминированную очистку от бюджета времени основного сценария.**
`apps/web/e2e/csv-import.compose.spec.ts`, функция `cleanupBySearch()` —
единственное изменение (используется во всех трёх `finally`-блоках файла,
строки 100/164/266 по номерам тестов). До правки один общий Playwright test
timeout (30000мс) делил бюджет между самим сценарием И `finally`-очисткой —
единичная задержка ответа сервера во время cleanup (по п. 5, независимо от
кода) проваливает тест уже ПОСЛЕ того, как сценарий сам успешно прошёл, и
делает такой провал неотличимым от настоящего зависания сценария. Добавлено:

- `test.setTimeout(test.info().timeout + CLEANUP_TIMEOUT_MS + 20_000)` в
  начале `cleanupBySearch()` — официальный идиоматический механизм
  Playwright именно для этого случая (`testInfo.setTimeout(testInfo.timeout

* N)`, задокументирован и типизирован в самом Playwright
`playwright/types/test.d.ts:2411/2686/5939`как канонический пример для
«медленного cleanup-хука»). Продлевает бюджет ТОЛЬКО из этой функции
(вызывается только из`finally`) — исходный test timeout, которым
по-прежнему ограничен сам сценарий через собственные
`expect(...).toBeVisible()`до вызова`cleanupBySearch()`, не увеличен.

- Явный `timeout: CLEANUP_TIMEOUT_MS` (40000мс) на каждый сетевой вызов
  внутри `cleanupBySearch()` (`GET` и каждый `DELETE`). `40000` — не
  круглое число «на случай чего»: это `statement_timeout` (35000мс,
  `apps/api/src/prisma/client.ts:59`, уже проверенная в предыдущих сессиях
  Stage 12 абсолютная верхняя граница, на которую Postgres сам может
  задержать ОДИН SQL-statement) плюс 5000мс запаса на сеть/диспетчеризацию.

**Почему это не маскировка, а структурное исправление.**

- Это не blind увеличение test timeout — расширение бюджета ограничено
  ИСКЛЮЧИТЕЛЬНО функцией `cleanupBySearch()`, вызываемой только в `finally`,
  ПОСЛЕ того как основной сценарий уже прошёл все свои
  `expect(...).toBeVisible()` с их собственными (не увеличенными) таймаутами
  — зависание/поломка самого сценария по-прежнему проваливает тест в
  прежнем эффективном окне.
- Значение не подобрано произвольно: `40000` = задокументированный и уже
  аудированный `statement_timeout` + фиксированный запас; расширение общего
  бюджета `CLEANUP_TIMEOUT_MS + 20_000` явно рассчитано на «GET + минимум
  один DELETE» с запасом, а не «увеличить, пока не позеленеет».
- Явный per-request `timeout` — не смягчение assertions и не retry: при
  реальном зависании тест по-прежнему падает, но теперь с точным,
  атрибутируемым сообщением («запрос очистки не ответил за N мс»,
  Playwright `APIRequestContext` timeout error) вместо неоднозначного
  «Test timeout of 30000ms exceeded», которое сейчас одинаково выглядит и
  при зависшем сценарии, и при зависшей очистке — это строго улучшает
  диагностируемость, а не скрывает дефект.
- `cleanupBySearch()`/бизнес-логика импорта не менялись; retries не
  добавлялись; assertions не ослаблены; тест не skip/disabled.
- Не устраняет (и не может устранить из кода этого репозитория) сам шум
  OS-планировщика shared-раннера — только ограничивает его влияние ОДНОЙ
  фазой теста (cleanup), где задержка ответа сервера не означает дефект
  сценария, и делает будущий рецидив, если он случится, немедленно
  отличимым по сообщению об ошибке.

**9. Регрессионный тест не добавлялся.** Причина зависания — не в
детерминированном коде приложения/теста (п. 2–5), а в шуме
CPU-планировщика shared CI-раннера, который не воспроизводится по
требованию (см. «Честно о границах доказательства» выше) — написать
детерминированный regression-тест на недетерминированное свойство
инфраструктуры хоста означает написать тест, который либо всегда проходит
(бесполезен), либо сам становится источником flakiness. Структурное
изменение (п. 8) — единственная проверяемая защита: два прогона clean-runner
gate после исправления (п. 10 ниже) — реальная эмпирическая проверка того,
что расширенный бюджет cleanup не ломает и не маскирует ничего в штатном
режиме.

**10. Прогоны этой сессии (после исправления, ноль изменений в
`apps/api/**`).**

| Проверка                                                                                                                                                                                                                                                                                                                                                                                          | Результат                                                                                                                                                                                                |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm --filter @finora/web exec tsc --noEmit`                                                                                                                                                                                                                                                                                                                                                     | PASS                                                                                                                                                                                                     |
| `pnpm eslint apps/web/e2e/csv-import.compose.spec.ts`                                                                                                                                                                                                                                                                                                                                             | PASS (0 warnings)                                                                                                                                                                                        |
| `pnpm exec prettier --check apps/web/e2e/csv-import.compose.spec.ts`                                                                                                                                                                                                                                                                                                                              | PASS                                                                                                                                                                                                     |
| Targeted `csv-import.compose.spec.ts --project=chromium --no-deps --workers=2 --retries=0` (та же concurrency, что упавший remote CI job), полный `restart api` + wait-for-healthy перед КАЖДЫМ из 20 прогонов                                                                                                                                                                                    | **PASS — 20/20 подряд, 0 timeout/flake, retries=0**                                                                                                                                                      |
| `node scripts/check-stage9-e2e.mjs` (родной gate: 20 прогонов ВСЕГО файла `csv-import.compose.spec.ts`, workers=1/2/4 по ротации, 80 test cases) против отдельного disposable Compose                                                                                                                                                                                                             | **PASS** — «20 последовательных запусков, 80 test cases…, workers=1/2/4, retries=0»                                                                                                                      |
| `pnpm test:e2e:auth` (`test-docker.mjs --browser`, ПОЛНЫЙ CI-эквивалентный clean-runner gate: чистая копия исходников во временный каталог, отдельный disposable Compose-проект, весь browser/Compose acceptance — auth/finance/budgets/dashboard/CSV wizard/export/mobile/responsive/axe, DB outage/recovery, embedded Stage 7 stress 20/20 и Stage 9 stress 20/20, dataset SHA-256 determinism) | **PASS**, exit code 0, 0 совпадений `FAIL` во всём логе (600 строк); итоговый dataset SHA-256 `55c7d9a5...` совпадает с эталонным из Stage 11/12; «Acceptance environment остановлен, его volume удалён» |
| `git status --short` после всех прогонов                                                                                                                                                                                                                                                                                                                                                          | чисто, только `M apps/web/e2e/csv-import.compose.spec.ts` — рабочее дерево без временных файлов                                                                                                          |

**11. Изменённые файлы.** Только `apps/web/e2e/csv-import.compose.spec.ts`
(+22/−0, добавлена константа `CLEANUP_TIMEOUT_MS` и продление таймаута
внутри `cleanupBySearch()`; сама сигнатура и логика функции — без изменений
кроме добавленных `timeout`). `apps/api/**` в этой сессии не менялся —
timeout-policy fix (`d029733`/`7d48d25`) уже подтверждён отдельным зелёным
remote CI (п. «Remote CI recheck» выше) и не тронут.

Commit и push не выполнялись в этой сессии — по действующему правилу
проекта (см. остальные записи REPORT.md), коммит делает пользователь
самостоятельно.

**STAGE 12 CI-РЕГРЕСС (`94e9f28`) ПРОДИАГНОСТИРОВАН ПО ПРЯМОЙ
TIMESTAMP-КОРРЕЛЯЦИИ ЛОГОВ: server-side исключён уликами (не
предположением), причина — CPU-планировщик shared remote-раннера (не CSS,
не регрессия timeout-policy fix'а из `d029733`/`7d48d25`, не необнаруженная
утечка соединений — отдельно перепроверено чтением `@prisma/client@7.10.0`
на предмет гонки `maxWait`). Структурное исправление
(`test.setTimeout`/per-request timeout в `cleanupBySearch()`) внедрено и
проверено: targeted 20/20 @ workers=2, полный Stage 9 stress 20/20, полный
CI-эквивалентный clean-runner gate — PASS. REMOTE CI RECHECK PENDING.**
