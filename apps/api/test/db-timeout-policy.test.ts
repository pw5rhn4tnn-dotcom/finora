...
await test('interactive transaction timeout короче ещё выполняющегося statement (< statement_timeout): следующий запрос через тот же pool не встаёт в зомби-очередь', async () => {
  const fixture = await databaseFixture();
