# Frontend Finora · Stages 3–7

Работающая адаптивная оболочка, общие UI primitives и светлая тема.
Stages 4–6 реализуют авторизацию, профиль, категории, операции и бюджеты.
Stage 7 показывает реальную месячную аналитику и Insights. Регулярные операции,
CSV и интерфейс аудита остаются предварительными разделами следующих этапов.
...
`features/dashboard` содержит generated query hook, композицию экрана и два SVG
chart-компонента; `OverviewPage` монтирует их с key владельца. Никаких новых
dependencies, handwritten HTTP или browser financial aggregation. Координаты
диаграмм используют Number только при отрисовке; доходы/расходы/проценты вычислены
API. Точные данные доступны списком категорий и таблицей шести месяцев.

Query key включает owner и период, AbortSignal передаётся в Orval и проверяется
после ответа до обработки 401. Общая cancel-before-invalidate Stage 5 обновляет
обзор после финансовых мутаций. Старые данные периода не сохраняются под новым
заголовком; error скрывает показатели и предлагает retry выбранного периода.
Пустой текущий месяц не скрывает доступную историю и бюджеты.

Responsive Grid использует доступную ширину колонок, включая 200% root font.
Семь viewport, keyboard, reduced motion, axe и screenshots проверяет
`dashboard.compose.spec.ts`. Его проект зависит только от immutable budgets-auth
setup; случаи имеют собственные category/transaction/budget с finally cleanup.
`pnpm test:e2e:stage7-stress` повторяет критические гонки 20 раз с workers 1/2/4
и retries=0. Setup sessions удаляются после stress; картинки используют outputPath.
Пороговые правила и границы API находятся в ARCHITECTURE §31.
