...
Stages 1–11 реализованы: операции, бюджеты, dashboard/Insights, регулярные
операции, импорт/экспорт CSV и read-only журнал изменений (`/audit-log`) с
читаемым до/после diff поверх аудита, который пишется с Stage 4. Stage 11
завершил security/hardening ревизию продукта; Stage 12 — воспроизводимая
сдача и пятиминутная демонстрация, описанная ниже.
...
Сценарий для быстрого показа продукта на demo-данных, без регистрации нового
аккаунта. Каждый шаг проверен вручную на собранных production-образах (Stage 12).

1. `docker compose up`, дождаться готовности сервисов (см. выше), открыть
   [http://localhost:8080](http://localhost:8080).
2. Войти demo-кнопкой «Личный профиль» (`personal@finora.example` /
   `Finora-Personal-2026!`).
3. **Обзор** — KPI выбранного месяца (доходы/расходы/разница/доля сбережений),
   расходы по категориям, тренд за 6 месяцев, Top-5, статус бюджетов и до
   четырёх наблюдений.
4. **Добавить операцию** — сохранить новый расход; на «Обзоре» и «Транзакциях»
   сумма и KPI обновляются сразу же.
5. **Бюджеты** — месяц с превышением лимита и месяц около лимита в одном списке.
6. **Регулярные операции** — активные MONTHLY-правила (зарплата, аренда,
   подписки) с датой следующей операции.
7. **Транзакции → Экспорт CSV**, затем **Импорт CSV** — мастер из четырёх шагов
   (файл → столбцы → категории/курсы → подтверждение).
8. **Журнал изменений** (`/audit-log`) — открыть «Подробнее» у записи и
   показать читаемый before/after diff.
9. Выйти и войти demo-кнопкой «Семейный профиль» (`family@finora.example` /
   `Finora-Family-2026!`) — независимые данные второго аккаунта.
10. Открыть [Swagger UI](http://localhost:8080/docs) — живой контракт API.
11. Остановить: `docker compose down` (данные сохраняются в volume) или
    `docker compose down -v` для полного сброса demo-БД.
...
**Stages 0–11 закоммичены и присутствуют в git-истории** (от `bb3ff1a` —
архитектурная документация до первого application code — до `f85d6ed`, Stage
11: Hardening & Testing). Текущий HEAD подтверждён зелёным remote GitHub
Actions run. Полная история remote CI по каждому Stage, включая найденные и
исправленные регрессии, зафиксирована в [REPORT.md](REPORT.md).
